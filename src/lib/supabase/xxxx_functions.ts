...
export interface ReportEntryParams {
  entry_id: string
  /**
   * 对齐 DB: entry_reports.reason_code
   * - 新值：price_incorrect | service_not_delivered | duplicate | fraud | expired | offensive | other
   * - 旧值（兼容）：inaccurate -> price_incorrect, fake -> fraud
   */
  reason: ReportReason
  details?: string
}
