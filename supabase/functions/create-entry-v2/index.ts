// supabase/functions/create-entry-v2/index.ts
...
> {
  ...
  if (!token) {
    return {
      ok: false,
      res: json<ApiErrorPayload>({ ok: false, error_code: 'AUTH_REQUIRED', message: 'Not signed in.' }, 401),
    }
  }
  ...
  if (!supabaseUrl || !anonKey) {
    return {
      ...
      res: json<ApiErrorPayload>(
        { ok: false, error_code: 'SERVER_MISCONFIG', message: 'Missing SUPABASE_URL or SUPABASE_ANON_KEY.' },
        500
      ),
    }
  }
  ...
  if (!r.ok) {
    return {
      ok: false,
      res: json<ApiErrorPayload>({ ok: false, error_code: 'AUTH_INVALID', message: 'Session expired or invalid.' }, 401),
    }
  }
...
serve(async (req: Request) => {
  ...
  if (req.method !== 'POST') {
    return json<ApiErrorPayload>({ ok: false, error_code: 'METHOD_NOT_ALLOWED', message: 'Use POST.' }, 405)
  }
