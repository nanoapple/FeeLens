// @ts-nocheck
// ==========================================
// Edge Function: approve-provider
...
const ApproveProviderSchema = z.object({
  ...
  action: z.enum(['approve', 'reject'], {
    errorMap: () => ({ message: '操作必须是 approve 或 reject' })
  }),
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const validation = ApproveProviderSchema.safeParse(body)
    ...
    // 3. 调用 RPC（不传 admin_id，RPC 内部用 auth.uid() + admin 角色校验）
    console.log(`管理员 ${user.id} 审核商家 ${data.provider_id}, action=${data.action}`)

    const { data: rpcResult, error: rpcError } = await supabase.rpc('approve_provider', {
