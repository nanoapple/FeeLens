...
const ResolveDisputeSchema = z.object({
  dispute_id: z.string().uuid('无效的争议 ID'),
  outcome: z.enum(['maintained', 'corrected', 'removed', 'partial_hidden'], {
    errorMap: () => ({ message: '结果必须是 maintained、corrected、removed 或 partial_hidden' })
  }),
  platform_response: z.string().min(1, '平台回应不能为空').max(5000),
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`管理员 ${user.id} 处理争议 ${data.dispute_id}, outcome=${data.outcome}`)

    const { data: rpcResult, error: rpcError } = await supabase.rpc('resolve_dispute', {
    ...
    if (rpcError) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Internal server error: 争议处理失败' }),
